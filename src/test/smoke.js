...
setTimeout(async () => {
  ...
  // ---- a step that the deal's contract does not have is refused, not silently applied ----
  {
    ...
    /* Отсеков пять: к четырём укрупнённым добавилась «Квалификация». Прежде эта стадия на
    ...
    check('доска · заявки в начале пути, договор и исход в конце',
      cols[0] === 'Квалификация' && cols[1] === 'Подбор и показы' && cols[2] === 'Переговоры' &&
  ...
  // ---- Волна 3: коммерческие предложения версиями --------------------------------------------
  {
    ...
    /* ---- Наследование согласия: разрешено по совпадению, а не запрещено по несовпадению ----
       Проверка «группа не client» дырява: `roleGroupOf` на роли, которой нет в справочнике,
       возвращает `client` по умолчанию — и опечатка «Менеджер застройщика» вместо
       «Менеджер девелопера» открывала участнику чужое согласие. Спрашиваем сам модуль,
       по одной роли на случай. */
    {
      const giver = { id: 'c_probe_consent', name: 'Клиент с согласием', consent: true };
      const opts = { dealClients: [giver] };
      const verdict = (role) => {
        const r = WS.audience.calculateAudience([{ id: 'p_probe', role: role, channel: 'wa' }], opts);
        return r.suitable.length ? 'допущен' : (r.excluded[0] || {}).reason;
      };
      check('согласие · сторона клиента наследует согласие клиента сделки',
        verdict('Супруг') === 'допущен', String(verdict('Супруг')));
      check('согласие · старое название роли переносится и наследует',
        verdict('Покупатель') === 'допущен', String(verdict('Покупатель')));
      check('согласие · другая сторона стола чужого согласия не получает',
        verdict('Собственник') === 'согласие принадлежит другому лицу' &&
        verdict('Агент-партнёр') === 'согласие принадлежит другому лицу',
        verdict('Собственник') + ' | ' + verdict('Агент-партнёр'));
      check('согласие · роль вне справочника не считается стороной клиента',
      ...
      const emitted = ['Собственник', 'Менеджер застройщика', 'Наблюдатель']
        .map(verdict).concat(['нет согласия', 'согласие унаследовано, источник его не имеет'])
        .filter((x) => x && x !== 'допущен');
      const unlisted = emitted.filter((x) => listed.indexOf(x) < 0);
      check('согласие · каждая выдаваемая причина исключения перечислена',
        unlisted.length === 0, unlisted.join(' | ') || 'все перечислены');
    }
  }
  ...
  /* ---- Подсказка, которую стенд предлагает, обязана быть им же разобрана -----------------
     «Какие задачи просрочены и по кому» — это ЕГО СОБСТВЕННАЯ подсказка, и на неё приходило
     «Такого у нас в данных нет», а следом перечислялось ровно то, о чём спросили. «Какие
     запросы горячие» отвечалось про сделки: признак «горячий» читался как признак сделки
     раньше, чем названная коллекция успевала его уточнить. Замкнутый круг в обоих случаях:
     система советует спросить то, на что не отвечает. */
  {
    WS.router.go('start');
    const ROOT = [[/запрос|заяв/, /запрос|заяв/], [/задач/, /задач/], [/сделк/, /сделк/],
      [/контакт|клиент/, /контакт|клиент/], [/объект/, /объект/]];
    const chips = (WS.agent.tools.followUps ? WS.agent.tools.followUps({}) : [])
      .map((c) => c.ask).filter(Boolean);
    check('подсказки · стенд вообще что-то предлагает', chips.length >= 2, String(chips.length));
    const deaf = chips.filter((q) =>
      /Такого у нас в данных нет|Не понял вопрос/.test((WS.agent.ask(q) || {}).text || ''));
    check('подсказки · каждую свою подсказку планировщик разбирает', deaf.length === 0,
      'не понял: ' + deaf.join(' | '));
    const wrong = chips.filter((q) => {
      const pair = ROOT.find((r) => r[0].test(q));
      if (!pair) return false;
      return !pair[1].test((WS.agent.ask(q) || {}).text || '');
    });
    check('подсказки · ответ про ту коллекцию, которую подсказка называет', wrong.length === 0,
      wrong.map((q) => q + ' -> ' + (WS.agent.ask(q).text || '').slice(0, 60)).join(' ; '));
  }
  ...
  // ---- Консьерж работает внутри карточки: без перехода и без потери экрана ----
  {
       ...
       и честно отвечала, что не поняла. Экран — такой же вход, как текст поручения. */
    {
      ...
      /* ---- Закрытая сделка: не «застряла», и двигать её некуда --------------------------
         Кросс-модельная вычитка нашла: следующий шаг считался без проверки исхода, и на
         успешно закрытой сделке и карточка, и Консьерж предлагали «Вернуться к сделке:
         позвонить, предложить следующий шаг · застряла 8 дн. в стадии» — совет открыть заново
         то, что закончено. Проверяется корень: следующий шаг берут из него четыре места. */
      {
        const won = (dd().deals || []).find((x) => x.stage === 'won');
        if (won) {
          const nx = WS.ui.dealNext(won.id);
          check('сделка · у закрытой сделки следующий шаг не зовёт открыть её заново',
            !!nx && !/Вернуться к сделке/.test(nx.action || '') && !/застряла/.test(nx.why || ''),
            JSON.stringify(nx));
          WS.ui.dealCard(won.id);
          const line = doc.querySelector('#app .view .plev-next');
          check('сделка · и карточка закрытой сделки говорит то же самое',
            !!line && !/застряла/.test(line.textContent),
            line ? line.textContent.replace(/\s+/g, ' ').slice(0, 120) : 'выделенной строки нет');
          /* Отсутствие строки «Мешает:» — отсутствие сработавшей проверки, и ничего больше.
             Ответ выводил из него три положительных факта и на сделке без задатка в записи
             утверждал, что задаток внесён. Проверяется на всех сделках сразу. */
          const lies = (dd().deals || []).filter((x) => !(x.deposit && x.deposit.paid)).filter((x) => {
            WS.ui.dealCard(x.id);
            return /задаток внес[её]н/i.test(WS.agent.ask('что мешает').text || '');
          });
          check('консьерж · «что мешает» не сочиняет факты, которых в записи нет',
            lies.length === 0, lies.map((x) => x.id).join(', '));
          /* И у ЗАКРЫТОЙ сделки — обеих, выигранной и проигранной. Для неё блок помех вообще не
             выполняется, поэтому «проверяются пять причин, не сработала ни одна» — тоже
             сочинение: не сработала ни одна, потому что ни одна и не запускалась. Исход при
             этом читается из записи: прежняя проверка искала «закрыт» в тексте справки, а это
             слово туда пишет только выигранная сделка — и проигранная проскакивала. */
          (dd().deals || []).filter((x) => x.stage === 'won' || x.stage === 'lost').forEach((x) => {
            WS.ui.dealCard(x.id);
            const a = WS.agent.ask('что мешает').text || '';
            check('консьерж · у закрытой сделки (' + x.stage + ') помехи не «проверяются» задним числом',
              /не отслеживаются/.test(a) && !/не сработала ни одна/.test(a), a.slice(0, 120));
          });
          /* Срок закрытой сделки не наследуется: `nextDue` остаётся тем, что стоял в работе, и
             «запросить отзыв» получало дедлайн касания, которого больше нет. В готовых данных
             у закрытых сделок срок пуст, поэтому переход проверяется переключением исхода. */
          {
            const live = (dd().deals || []).find((x) => x.nextDue && x.stage !== 'won' && x.stage !== 'lost');
            if (live) {
              const was = live.stage;
              live.stage = 'won';
              const nx = WS.ui.dealNext(live.id);
              live.stage = was;
              check('сделка · закрытие не переносит на новый шаг срок прежнего касания',
                !!nx && !nx.due && !nx.over, JSON.stringify(nx) + ' при nextDue=' + JSON.stringify(live.nextDue));
            }
          }
          WS.ui.dealCard('d_anna');
        }
      }
      /* ---- Вопрос про множество, заданный с карточки ------------------------------------
         «Статус задач» и «какие сделки активны» возвращались справкой по открытой сделке:
         «задач» — форма только множественная, а «какие» вовсе не было среди вопросительных
         слов планировщика. Ответ должен быть счётом по коллекции, а не записью под курсором. */
      /* «Какими» знал только разбор области вопроса — вопросительные слова планировщика и
         счётчик про него не знали, и «какими задачами нужно заняться» уходило в «такого у нас
         в данных нет». Три списка слов об одном и том же обязаны совпадать. */
      ['статус задач', 'какие сделки активны', 'какие задачи просрочены',
       'какими задачами нужно заняться'].forEach((q) => {
        const a = WS.agent.ask(q);
        check('консьерж · «' + q + '» отвечается по коллекции, а не по открытой сделке',
          /\d/.test(a.text || '') && (a.text || '').indexOf(dl.title) < 0 &&
          (a.text || '').indexOf('Такого у нас в данных нет') < 0, (a.text || '').slice(0, 120));
      });
  ...
  // ничем не удерживался, он и уехал. Дальше держится здесь.
  {
       ...
       предложить · первый шаг, — и что она НЕ является открытой сделкой. */
    {
      ...
      /* Двенадцать: «возможность заперта согласием» уехала в «Требует внимания». По
         определению принципала это не сделка — предложить клиенту сейчас нечего, сначала надо
         получить право писать. Заодно ушла и сама формулировка: «возможность» ничего не делает
         и не может быть заперта, а «заперта согласием» говорит обратное истине — запирает не
         согласие, а его отсутствие. */
      const RULES_EXPECTED = ['lease_window', 'repeat_offplan', 'yield_gap', 'handover_window',
        'block_match', 'lost_return', 'management_upsell',
      ...
      check('Пульс · каждый из двенадцати разборов нашёл повод, ни один не выпал',
    ...
    /* ---- Инсайты — знание о рынке, а не работа с клиентом ---------------------------
       Определение принципала: инсайт — результат аналитики вне работы с конкретным клиентом
       плюс гипотеза, кому это пригодится. У него НЕТ адресата — есть применимость. Всё, что
       стояло здесь раньше, оказалось задачами и уехало в «Мои дела».

       Проверяется анатомия: заголовок с названным деятелем, отсылка со ссылкой, считаемое
       «у нас» и ход. И два правила, без которых жанр снова превратится в текст. */
    {
      const ins2 = WS.ui.pulseInsights();
      check('Инсайты · их несколько и каждый несёт все четыре части',
        ins2.length >= 5 && ins2.every((x) => x.title && x.src && x.url && x.seen && x.move && x.until),
        ins2.length + ' карточек, без части: ' +
        (ins2.filter((x) => !(x.title && x.src && x.url && x.seen && x.move && x.until)).map((x) => x.id).join(', ') || '—'));
      /* Ссылка обязательна: без первоисточника карточка неотличима от правдоподобного текста,
         на чём мы уже обожглись — четыре прежних «инсайта» были именно им. */
      const noUrl = ins2.filter((x) => !/^https?:\/\//.test(String(x.url || '')));
      /* Сломанная формулировка не должна вернуться ни в один раздел. «Возможность заперта
         согласием» ломалась дважды: подлежащим стала абстракция, а творительный падеж сказал
         обратное истине — запирает отсутствие согласия, а не согласие. */
      {
        const all = WS.ui.pulseProspectList().map((p) => p.title)
          .concat(WS.ui.pulseAlerts().map((a2) => a2[1]))
          .concat(ins2.map((x) => x.title));
        ...
        check('Пульс · согласие на связь стоит задачей, а не перспективной сделкой',
          ...
          consentTask ? consentTask[1] : 'задачи нет');
      }
      check('Инсайты · у каждого стоит ссылка на первоисточник',
        noUrl.length === 0, noUrl.map((x) => x.id).join(', '));
      /* Заголовок называет ДЕЯТЕЛЯ. Подлежащим не может быть обстоятельство: «рассрочка»,
         «волна», «подписание», «окно» ничего не делают, и фраза с таким подлежащим прячет
         того, кто действует на самом деле. */
      /* Граница слова здесь — ПРОБЕЛ, а не \b: в JavaScript \b определена через \w, то есть
         [A-Za-z0-9_], и после кириллицы не срабатывает вовсе. Первая версия проверки пропустила
         заголовок «Рассрочка больше не откладывает…» именно поэтому — та самая ловушка, которая
         уже описана в этом файле для \w. */
      const FAKE_SUBJECTS = /^(Рассрочка|Волна|Подписание|Окно|Возможность|Ситуация|Тренд)(\s|$)/i;
      const badSubj = ins2.filter((x) => FAKE_SUBJECTS.test(x.title));
      check('Инсайты · в заголовке действует тот, кто действительно действует',
        badSubj.length === 0, badSubj.map((x) => x.title).join(' | '));
      const longTitles = ins2.filter((x) => x.title.length > 72);
      check('Инсайты · заголовок читается за раз — не длиннее семидесяти двух знаков',
        longTitles.length === 0, longTitles.map((x) => x.title.length + ': ' + x.title).join(' | '));
      /* Число в заголовке обязано СЧИТАТЬСЯ. Написанное руками, оно расходится с расчётом в
         первый же день: «Пять районов из шести» стояло над строкой «7 из 9». */
      {
        const rows = (WS.query.run({ from: 'market' }) || {}).rows || [];
        const card = ins2.find((x) => /дают доходность ниже/.test(x.title));
        const below = rows.filter((m) => m.доходностьПроцент < 6.58).length;
        check('Инсайты · число в заголовке посчитано, а не написано',
          !!card && card.title.indexOf(String(below)) === 0 && card.title.indexOf(String(rows.length)) > 0,
          card ? card.title + ' · в данных ' + below + ' из ' + rows.length : 'карточки нет');
      }
      /* «У нас» — единственная считаемая часть, и она обязана меняться вслед за базой.
         Уберём у всех клиентов признак «важна виза» — карточка про визовый порог должна
         исчезнуть, а не остаться с прежним текстом. */
      {
        const cls = dd().clients || [];
        const was = cls.map((c) => c.visaGoal);
        cls.forEach((c) => { c.visaGoal = false; });
        const after = WS.ui.pulseInsights();
        cls.forEach((c, i) => { c.visaGoal = was[i]; });
        check('Инсайты · карточка исчезает, когда в базе не к кому её приложить',
          after.length === ins2.length - 1 && !after.some((x) => x.id === 'ins_visa_rule'),
          after.length + ' против ' + ins2.length);
      }
      /* И наоборот: «у нас» пересчитывается, а не запоминается. */
      {
        const c3 = (dd().clients || []).find((x) => x.visaGoal && x.budget && x.budget < 2000000);
        if (c3) {
          const wasB = c3.budget;
          const before = (WS.ui.pulseInsights().find((x) => x.id === 'ins_visa_rule') || {}).seen || '';
          c3.budget = 1990000;
          const nowSeen = (WS.ui.pulseInsights().find((x) => x.id === 'ins_visa_rule') || {}).seen || '';
          c3.budget = wasB;
          /* Сравнение по ЦИФРАМ: разряды разделены не обычным пробелом, и поиск строки
             «10 000» в тексте не находит «10 000», набранное типографским разделителем. Тот
             же класс, что уже ломал две проверки на числах. */
          const digitsOf = (x) => String(x).replace(/[^0-9]/g, '');
          check('Инсайты · «у нас» пересчитывается по базе, а не написано один раз',
            before !== nowSeen && digitsOf(nowSeen).indexOf(digitsOf('1990000')) >= 0,
            nowSeen.slice(0, 90));
        }
      }
      /* На экране карточка показывает всё это, а не только заголовок. */
      {
        const box = openPulse('insights');
        const one = box.querySelector('.ins');
        check('Инсайты · на экране видно жанр, срок, откуда, у нас и ход',
          !!one && !!one.querySelector('.ins-kind') && !!one.querySelector('.ins-until') &&
          !!one.querySelector('.ins-row.is-ours') && !!one.querySelector('a[href^="http"]') &&
          one.querySelectorAll('.ins-row').length === 3,
    ...
    /* ---- «Требует внимания» считается, а не написано --------------------------------
     ...
     Два самых дорогих пробела закрываются здесь. */
  {
    ...
    if (WS.engine.turnState) {
      ...
      /* ОХРАНА ОТМЕНЫ НЕ ЗАКРЕПЛЯЕТСЯ, и это не упущение, а свойство кода.

         Мутация её условия выживает потому, что различить версии негде:
           — хода нет → обе версии выходят по первому условию;
           — ход идёт → обе продолжают и отменяют;
           — ход уже отменён → мутант перерисовывает ТУ ЖЕ карточку;
           — ход завершён успешно → такого состояния не существует: endTurn удаляет
             ход из хранилища, оставляя только отменённые и упавшие.

         То есть условие защищает от недостижимого. Убирать его не надо — оно станет
         значимым, как только завершённые ходы начнут храниться. Но гоняться за проверкой
         на него тоже не надо: это эквивалентная подмена, а не неизмеренное поведение. */
  ...
  {
    /* Спрашивать надо БЕЗ открытой записи: стоя на заявке, стенд уточняет
       про неё, а не отвечает про район, — и это верное поведение, а не дефект. */
    WS.storeApi.resetAll();
    if (WS.engine.closeThread) WS.engine.closeThread();
    const noArea = WS.agent.ask('что по рынку в Damac Hills');
    const txt = String(noArea.text || '');
    check('район вне покрытия · сказано честно, без приближения по соседнему',
      /нет/.test(txt) && /Business Bay/.test(txt), txt.slice(0, 70));
    check('район вне покрытия · предложен партнёрский путь, а не только список того, что есть',
      /партнёр/i.test(txt), txt.slice(-90));
  }
  ...
  /* ---- Модель видит БАЗУ ЦЕЛИКОМ ----
     ...
     заявок и «показано 7 из 15» объектов. Отвечала она честно и о неполноте
     предупреждала, но подбор по половине инвентаря — это не подбор.

     Порог живёт НА СЕРВЕРЕ, а размер дайджеста растёт НА СТЕНДЕ — именно
     поэтому они разошлись тихо. Проверка читает оба числа и сверяет их между собой:
     иначе следующее наполнение инвентаря сломает это так же молча. */
  {
    WS.storeApi.resetAll();
    const dg = WS.live.digest();
    const dgLen = JSON.stringify(dg).length;

    const proxySrc = fs.readFileSync(path.join(__dirname, '..', '..', 'server', 'proxy.js'), 'utf8');
    const capOf = (name) => {
      const m = proxySrc.match(new RegExp(name + ':\\s*(\\d+)\\s*\\*\\s*(\\d+)'));
      return m ? Number(m[1]) * Number(m[2]) : 0;
    };
    const digestCap = capOf('maxDigestChars');
    const bodyCap = capOf('maxBody');

    check('база целиком · дайджест влезает в потолок с запасом втрое',
      digestCap > 0 && dgLen * 3 < digestCap,
      dgLen + ' символов против потолка ' + digestCap);

    /* Дайджест едет В ТЕЛЕ ЗАПРОСА: если тело уже дайджеста, поднятый
       потолок ничего не даст — запрос отобьют на входе. */
    check('база целиком · тело запроса шире самого дайджеста',
      bodyCap > digestCap, 'тело ' + bodyCap + ' против дайджеста ' + digestCap);

    check('база целиком · все объекты и заявки в дайджесте',
      (dg.объекты || []).length === (dd().objects || []).length &&
      (dg.заявки || []).length === (dd().requests || []).length,
      (dg.объекты || []).length + '/' + (dd().objects || []).length + ' объектов');

    /* Четыре ответа из семнадцати заканчивались фразой «цену метра я не считал —
       скажите, посчитаю». Модель была права по-своему: писать числа от себя ей
       запрещено, а готового в данных не было. Теперь считает код — теми же
       функциями, какими считает для экрана, и расхождения между ними быть не может. */
    const withSize = (dg.объекты || []).filter((o) => o.площадь && o.цена);
    check('база целиком · цена за метр и за фут посчитаны до модели',
      withSize.length > 0 && withSize.every((o) =>
        o.цена_за_метр === Math.round(o.цена / o.площадь) &&
        o.цена_за_фут === Math.round(o.цена / WS.sqft(o.площадь))),
      withSize.length ? withSize[0].цена_за_метр + ' за м² · ' + withSize[0].цена_за_фут + ' за фт²' : 'нет');

    /* Сервисный сбор котируется за фут в год, а клиент спрашивает, сколько
       это в год деньгами. Экран умел это считать, модель — нет. */
    const withSc = (dg.объекты || []).filter((o) => o.сервисный_сбор_в_год);
    check('база целиком · сервисный сбор уходит модели суммой за год',
      withSc.length > 0 && withSc.every((o) =>
        o.сервисный_сбор_в_год ===
        WS.ui.objServiceYear((dd().objects || []).find((x) => x.id === o.id))),
      withSc.length ? withSc[0].название + ': ' + withSc[0].сервисный_сбор_в_год : 'нет');

    /* Дифференциальная проверка: поле должно СЧИТАТЬСЯ, а не лежать
       записанным в фикстурах — иначе при изменении цены оно соврёт. */
    const probeObj = (dd().objects || []).find((o) => o.size);
    const before = WS.live.digest().объекты.find((o) => o.id === probeObj.id).цена_за_метр;
    probeObj.price = probeObj.price * 2;
    const after = WS.live.digest().объекты.find((o) => o.id === probeObj.id).цена_за_метр;
    check('база целиком · цена за метр считается, а не хранится',
      after === before * 2 || Math.abs(after - before * 2) <= 1, before + ' → ' + after);
    probeObj.price = probeObj.price / 2;

    /* Сервисный сбор в Дубае котируется за фут в год. Без площади в футах
       модель просила эту цифру у брокера — у стенда она была всё это время. */
    const sized = (dg.объекты || []).filter((o) => o.площадь);
    check('база целиком · площадь уходит модели и в футах',
      sized.length > 0 && sized.every((o) => o.площадь_фт === WS.sqft(o.площадь)),
      sized.length ? sized[0].площадь + ' м² → ' + sized[0].площадь_фт + ' фт²' : 'нет объектов с площадью');
  ...
  /* ---- Площадь читается в двух единицах сразу ----
     ...
     языком договора. Одна единица на экране — это пересчёт в уме у того, кому мы эту
     работу обещали снять. */
  {
    WS.storeApi.resetAll();

    check('единицы · пересчёт в футы верен',
      WS.sqft(100) === 1076 && WS.sqft(112) === 1206, WS.sqft(100) + ' / ' + WS.sqft(112));

    check('единицы · по-русски ведут метры, футы рядом',
      /^112\sм²\s·\s1206\ssqft$/.test(WS.area(112)), WS.area(112));

    check('единицы · по-английски ведут футы, метры рядом',
      /^1206\ssqft\s·\s112\sm²$/.test(WS.area(112, true)), WS.area(112, true));

    /* Пустое значение не должно читаться как «0 м² · 0 sqft» — это утверждение
       о площади, которого мы не делали. */
    check('единицы · неизвестная площадь не притворяется нулём',
      WS.area(0) === '—' && WS.area(null) === '—' && WS.areaHtml(undefined) === '—',
      WS.area(0) + ' / ' + WS.areaHtml(undefined));

    /* В разметке вторая единица приглушена, иначе два числа рядом читаются
       как две разные величины. */
    check('единицы · вторая единица помечена как вторая',
      /<span class="u2">1206 sqft<\/span>/.test(WS.areaHtml(112)), WS.areaHtml(112));

    /* Главное — не функция, а то, что ею действительно пользуются на экранах.
       Проверяется карточка объекта — первый экран, куда смотрит брокер. */
    const anyObj = (dd().objects || []).find((o) => o.size);
    WS.ui.objectCard(anyObj.id);
    const objHtml = doc.getElementById('app').innerHTML;
    const m2 = String(anyObj.size); const ft = String(WS.sqft(anyObj.size));

    /* Каждое место — своя проверка, и каждая — на том экране, где это место
       действительно рисуется. Общее «где-то на экране есть футы» пропустило мутацию:
       площадь показывается в нескольких местах, и потеря одного проходит незамеченной. */
    check('единицы · плитка «площадь» на карточке несёт обе единицы',
      objHtml.indexOf('osum-v">' + m2 + ' м²<span class="u2">' + ft + ' sqft') >= 0,
      'плитка ' + m2);

    check('единицы · описание одной строкой несёт обе единицы',
      objHtml.indexOf(m2 + ' м² · ' + ft + ' sqft') >= 0, 'строка ' + m2);

    /* Список объектов — другой экран со своей разметкой и первое, на что
       брокер смотрит, перебирая инвентарь. */
    WS.router.go('objects');
    const listHtml = doc.getElementById('app').innerHTML;
    check('единицы · в списке объектов площадь стоит в двух единицах',
      /osv">\d+ м²<span class="u2">\d+ sqft<\/span>/.test(listHtml),
      (listHtml.match(/osv">[^<]*м²[^!]{0,40}/) || ['нет плитки'])[0]);

    check('единицы · в строке списка тип назван в двух единицах',
      /· \d+ м² · \d+ sqft/.test(listHtml), 'строка списка');
  ...
  /* ---- КП не подписывают ----
     ...
     Живой брокер спросил, что значит «отправить клиенту на подпись». Коммерческое
     предложение не подписывают — в Дубае подписывают Form B, Form F и Form I.
     Назвать действие неверно — значит научить брокера неверной механике сделки. */
  {
    WS.storeApi.resetAll();
    WS.ui.openKp();
    const kpHtml = doc.getElementById('modal').innerHTML;
    check('КП · отправка не называется подписанием',
      kpHtml.indexOf('на подпись</button>') < 0 &&
      kpHtml.indexOf('Отправить клиенту') >= 0,
      'кнопка в подвале КП');
    check('КП · сказано, где подпись будет на самом деле',
      /Form B|Form F/.test(kpHtml), 'ссылка на настоящие формы');
    WS.ui.closeModal();
  }
  ...
  /* ---- Сборы и пороги: число без происхождения не живёт ----
     ...
     без даты, вне контроля версий. Подтвердить такое нельзя, а устаревшее не видно вовсе —
     стенд продолжает называть старое число уверенным голосом, а брокер повторяет его клиенту.

     Теперь они живут по тому же правилу, что срез рынка. Проверка стоит на том, что
     тариф БЕЗ происхождения не может появиться незамеченным. */
  {
    const TT = (WS.fixtures && WS.fixtures.TARIFFS) || [];
    check('сборы · тарифы заведены в данных, а не в тексте инструкции',
      TT.length >= 8, 'записей ' + TT.length);

    const BASES = ['official', 'industry', 'per_deal'];
    const noBasis = TT.filter((t) => BASES.indexOf(t.basis) < 0);
    check('сборы · у каждого назван род происхождения',
      noBasis.length === 0, noBasis.map((t) => t.id + '=' + t.basis).join(', '));

    /* Источник обязан быть у КАЖДОГО тарифа, включая зависящие от сделки.
       Пустое поле читается как «происхождения нет», хотя оно есть — просто не публикация,
       а застройщик по сделке. На этом споткнулся разбор: увидел null и не дошёл до оговорки. */
    const noSrc = TT.filter((t) => !t.source);
    check('сборы · источник назван у каждого, а не оставлен пустым',
      noSrc.length === 0, noSrc.map((t) => t.id).join(', '));

    const noDate = TT.filter((t) => !t.asOf);
    check('сборы · у каждого есть дата проверки',
      noDate.length === 0, noDate.map((t) => t.id).join(', '));

    /* Официальный тариф обязан называть ведомство. Без этого он не официальный,
       а просто число, которому поверили. */
    const badOfficial = TT.filter((t) => t.basis === 'official' && !t.source);
    check('сборы · официальный тариф называет ведомство',
      badOfficial.length === 0, badOfficial.map((t) => t.id).join(', '));

    /* А то, что зависит от сделки, обязано нести оговорку: иначе модель подаст
       величину одного застройщика как общее правило — ровно та ошибка, что была
       с порогом переуступки. */
    const badPerDeal = TT.filter((t) => t.basis === 'per_deal' && !t.оговорка);
    check('сборы · зависящее от сделки несёт оговорку',
      badPerDeal.length === 0, badPerDeal.map((t) => t.id).join(', '));

    /* Комиссия на первичке — ноль для покупателя. Эта строка заведена потому,
       что первая версия правила вписала ему наши 2% и завысила смету входа. */
    const prim = TT.find((t) => t.id === 't_agency_primary');
    check('сборы · на первичке комиссию платит застройщик, а не покупатель',
      !!prim && prim.кто_платит === 'застройщик' && prim.сумма === 0,
      prim ? prim.кто_платит + ' / ' + prim.сумма : 'записи нет');

    /* Порог переуступки считается по ОПЛАЧЕННОМУ — на подмене этого планом
       стенд однажды построил рекомендацию «выходить до сдачи». */
    const thr = TT.find((t) => t.id === 't_assign_threshold');
    check('сборы · порог переуступки — по оплаченному, а не по плану',
      !!thr && /ФАКТИЧЕСКИ|оплачен/.test(String(thr.величина) + ' ' + String(thr.оговорка)),
      thr ? String(thr.величина) : 'записи нет');

    /* И они обязаны доезжать до модели: тариф, который есть в данных, но не в
       дайджесте, модель возьмёт из памяти или из поиска. */
    const dg = WS.live && WS.live.digest ? WS.live.digest() : {};
    const inDigest = dg['сборы_и_пороги'] || [];
    check('сборы · уезжают модели вместе с происхождением',
      inDigest.length === TT.length && inDigest.every((t) => !!t.basis && !!t.asOf),
      'в данных ' + TT.length + ', в дайджесте ' + inDigest.length);
  }
     ...
     планировку юнита картинкой, и чтобы всё это шло голосом в одно нажатие. */
  {
    ...
    /* Главное правило целого класса дефектов: НИ ОДИН документ, уходящий клиенту,
       не показывает наше вознаграждение. Сначала комиссия решала порядок в подборке,
       потом обнаружилась колонкой в КП. Проверка стоит на обоих документах сразу. */
    WS.ui.closeModal();
    WS.ui.openReqKp('r_anna');
    const kh = doc.getElementById('modal').innerHTML;
    const kt = kh.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ');
    check('КП · в документе клиента нет нашей комиссии',
      kh.indexOf('kp-doc') >= 0 && !/<th[^>]*>\s*Комиссия/.test(kh), kt.slice(0, 110));
    check('КП · вознаграждение показано БРОКЕРУ и помечено как внутреннее',
      /kp-internal/.test(kh) && /Видно только вам/.test(kt), 'блок внутренней строки');
    check('КП · на месте комиссии — то, на чём решает покупатель',
      /Готовность/.test(kt), kt.slice(0, 110));
  ...
  /* ---- Консьерж знает, на каком экране стоит брокер ----
     ...
     Аналитика, сам Консьерж) модели уезжало только название треда, и «что здесь?» с Пульса
     возвращало описание объекта, закрытого две минуты назад. Проверено на живом стенде.
     Теперь в описание разговора входит и текущий экран. */
  {
    WS.engine.bindThread('object:o_bayline', 'Bayline Terraces, Unit 1603', 'building');
    WS.router.go('start');
    const onPulse = (WS.live.scope() || {}).о_чём || '';
    check('Консьерж · на экране без записи модель узнаёт, где стоит брокер',
      /Пульс/.test(onPulse) && /Bayline/.test(onPulse), onPulse.slice(0, 160));

    const nd = (dd().deals || []).find((d) => d.clientId);
    WS.ui.dealCard(nd.id);
    const onCard = (WS.live.scope() || {}).о_чём || '';
    check('Консьерж · на карточке контекстом остаётся сама запись',
