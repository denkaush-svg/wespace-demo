...
(function (WS) {
  ...
  document.addEventListener('click', (e) => {
    ...
    // Правая часть карточки — единственное, что меняется: история и разговор с Консьержем
    // раскрываются в ней, а левая колонка с условиями остаётся на месте.
    if (d.rightpane) { store.rightPane = d.rightpane === 'off' ? null : d.rightpane; return api.emit(); }
    if (d.prosp) {
    ...
    if (d.cglang) { store.cgLang = d.cglang; return api.emit(); }
    if (d.cgdoclang) { store.cgDocLang = d.cgdoclang; return api.emit(); }
  ...
  function handleAct(act, t) {
    switch (act) {
      ...
      case 'kpSend': {
        ...
        WS.ui.closeModal();
        /* Ни «на подпись» — КП не договор, — ни внутреннего кода сценария: «A2» на
           экране у брокера означает ровно ничего. */
