...
(function (WS) {
  ...
  const EVENTS = [
    ...
    },
    {
      ...
      title: 'Ответ клиента на КП — Анна Петрова',
      sub: 'Анна ответила на отправленное вчера КП',
      thread: 'deal:d_anna', tlabel: 'Анна Петрова · сделка', ticon: 'users',
      say: 'Анна ответила на КП: <b>понравился Palm Court</b>, но <b>смущает первый платёж</b>. Определил сделку и объект, подготовил обновление и график платежей. Есть неоднозначность в сроке — <b>подсветил, не решаю сам</b>.',
      mark: 'prepared',
      effects: [{ op: 'updateDeal', id: 'd_anna', patch: { tags: ['ответ на КП', 'Palm Court 704'], sub: 'Palm Court 704 · возражение: первый платёж' } }],
      changed: ['Реакция клиента распознана', 'Сделка: подготовлено обновление (Переговоры)', 'Готов график платежей (черновик)', 'Неоднозначность срока — помечена для агента'],
      open: { scn: 'G3' },
