...
(function (WS) {
  ...
  function digest() {
    ...
    return {
      ...
      /* Сборы и пороги едут КАК ДАННЫЕ, а не текстом в инструкции. У каждого числа
         есть происхождение и дата — тогда модель может назвать их вслух, а брокеру есть чем
         ответить клиенту. В инструкции они были без источника, вне контроля версий и без
         возможности увидеть, что тариф устарел. */
      сборы_и_пороги: (WS.fixtures && WS.fixtures.TARIFFS) || null,
      объекты: take('objects', (o) => ({
        id: o.id, название: o.name, район: o.area, цена: o.price, площадь: o.size,
        /* Сервисный сбор в Дубае котируется ЗА ФУТ в год, а площадь лежала только
           в метрах. В стенограмме это стоило вот чего: модель попросила брокера
           назвать площадь в футах — цифру, которая у стенда была всё это время. */
        площадь_фт: o.size ? WS.sqft(o.size) : null,
        /* Посчитанное — в данных, а не в голове модели. В четырёх ответах из семнадцати
           модель заканчивала фразой «цену метра я не считал — скажите, посчитаю». И была
           права по-своему: писать числа от себя ей запрещено, а готового в данных не было.
           Считает код — теми же функциями, какими считает для экрана. */
        цена_за_метр: WS.ui.objPerM2(o),
        цена_за_фут: WS.ui.objPerSqft(o),
        отклонение_от_среза_процент: WS.ui.objPriceGap(o),
  ...
  function scope() {
    ...
    const about = rec
      ? [sc.экран + ' «' + rec.название + '»', rec.клиент ? 'клиент ' + rec.клиент : null,
        ...
        .filter(Boolean).join(', ')
      /* Экран без записи — Пульс, Аналитика, сам Консьерж — раньше отдавал модели ТОЛЬКО
         название треда, то есть последней карточки, с которой брокер ушёл. Проверено на живом
         стенде: стоя на Пульсе и спросив «что здесь?», брокер получал описание объекта Bayline,
         закрытого две минуты назад. Разговор при этом остаётся привязанным к записи законно —
         тред и есть переписка по ней, — так что подменять его нельзя; не хватало второго факта:
         где брокер стоит прямо сейчас. Теперь едут оба, и «здесь» разрешается по экрану. */
      : (sc && sc.экран
        /* Порядок здесь — не стиль. Первая версия правки добавляла экран ВТОРЫМ, после названия
           треда, и модель продолжала отвечать про запись: то, что стоит первым, читается как
           подлежащее. Проверено на живом стенде дважды. Теперь экран назван первым, а открытая
           переписка отодвинута и снабжена условием, когда её вообще уместно поминать. */
        ? 'брокер на экране «' + sc.экран + '», записи на нём не открыто — вопрос без явного ' +
          'подлежащего задан про этот экран, про него и отвечайте' +
          (th.label ? '. Открытая переписка ведётся по «' + th.label +
            '» — поминайте её только если спросили именно про неё' : '')
        : (th.label || th.id));
