...
(function (WS) {
  ...
  const MANY = new RegExp(['сколько|скольк|всего|все\\s|всех|список|перечисл|в работе|по всем|у меня|воронк|конверси',
    ...
    'контактов|контактам|клиентов|клиентам|компаний|компаниям|задачам|задачах'].join('|'), 'i');
  ...
  function screenAnswer(text) {
    ...
    if (rec.тип === 'сделка') {
      ...
      if (ASK_BLOCK.test(t)) {
        ...
        return say(stop
          ? 'По ' + named + ' ' + stop.charAt(0).toLowerCase() + stop.slice(1)
          : 'По ' + named + ' помех не отмечено: касание не просрочено, задаток внесён, ' +
            'расхождений по полям нет. Ближайшее дело — ' + nextPhrase(ui, rec) + '.');
      }
      if (ASK_NEXT.test(t)) {
      ...
      if (ASK_FIELD.test(t) && !ASK_BRIEF.test(t)) {
        ...
        if (/срок|дедлайн|касани/.test(t) && rec.срок_следующего_шага) {
          return say('Следующее касание по ' + named + ' — ' + rec.срок_следующего_шага + '.');
        }
      }
