...
(function (WS) {
  ...
  function screenAnswer(text) {
    ...
    if (rec.тип === 'сделка') {
      ...
      if (ASK_BLOCK.test(t)) {
        ...
        if (/закрыт/i.test(brief.join(' '))) {
          return say('Сделка «' + rec.название + '» закрыта — помехи по ней больше не отслеживаются.');
        }
        return say('По ' + named + ' помех не отмечено. Проверяются пять причин — просроченное ' +
          'касание, невнесённый задаток, расхождение по полям, незакрытый шаг договора и ' +
  ...
  const RE = {
    ...
    ask: /сколько|скольк|какая|какой|каких|какие|каков|сумма|итог|статус|состояни|что\s+(по|с|у|просроч|горит|дальше|мешает)|покажи|дай/,
  };
  ...
  function deterministicHead(text) {
    ...
    if (RE.ask.test(t) || /комисси|конверси|воронк/.test(t)) {
      ...
      const QUANTITY = /сколько|скольк|количеств|сумм|итог|всего|объ[её]м|покажи|дай|список|выведи|статус|состояни|каки[ех]|что\s|чего\s/i;
