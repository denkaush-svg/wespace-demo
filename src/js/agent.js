...
(function (WS) {
  ...
  const KIND_WORD = { 'сделка': /сделк/i, 'запрос': /заявк|запрос/i, 'контакт': /контакт|клиент/i,
  ...
  function screenScoped(text) {
    ...
    // Назвали другого человека — значит, спрашивают не про открытую запись.
    if (named && named.name && rec.клиент && lc(named.name) !== lc(rec.клиент)) return null;
    // «Сколько сделок в работе», «покажи все сделки» — вопрос про множество, а не про открытую
    // запись. Указательное слово перевешивает: «сколько задач по этой сделке» — снова про неё.
    const GLOBAL = /сколько|скольк|всего|все\s|всех|список|в работе|по всем|у меня|воронк|конверси/i;
    if (GLOBAL.test(t) && !DEMONSTRATIVE.test(t)) return null;
    const kw = KIND_WORD[rec.тип];
    if (!DEMONSTRATIVE.test(t) && !(kw && kw.test(t))) return null;
    return rec;
  }
  ...
  function orient(text) {
    ...
    const scrRec = (WS.ui && WS.ui.screenContext) ? (WS.ui.screenContext().запись || null) : null;
    const lines = askedArea
      ...
      : scrRec
        ? ['Не понял вопрос. Вы стоите на записи «' + scrRec.название + '» — уточните, что по ней нужно:',
           'бриф к звонку, что мешает закрыть, что просрочено, история переписки, следующий шаг.']
  ...
  function deterministicHead(text) {
    ...
    if (RE.ask.test(t) || /комисси|конверси|воронк/.test(t)) {
      ...
      const QUANTITY = /сколько|скольк|количеств|сумм|итог|всего|объ[её]м|покажи|дай|список|выведи|статус|состояни/i;
