...
(function (WS) {
  ...
  function orient(text) {
    ...
    /* Отрицание обязано заканчиваться действием. Район без покрытия — не тупик,
       а повод для co-broking: так это работает у живого брокера каждый день. Три живых
       вопроса из четырёх упёрлись именно в «у нас этого нет» без продолжения. */
    const lines = askedArea
      ? ['Этого района в нашем срезе рынка нет. Есть: ' + areas.join(', ') + '.',
         'Разобрать могу любой из них — цену за метр, доходность, срок экспозиции.',
         'А если клиенту нужен именно этот район — это не тупик, а повод пойти через партнёров: откройте Сеть и запросите инвентарь у тех, кто там работает.']
      : scrRec
