...
(function (WS) {
  ...
  const GLOBAL = /сколько|скольк|всего|все\s|всех|список|перечисл|покажи|показать|дай |выведи|в работе|по всем|у меня|воронк|конверси/i;
  ...
  const RE = {
    ...
    ask: /сколько|скольк|какая|какой|каков|сумма|итог|статус|состояни|что\s+(по|с|у|просроч|горит|дальше|мешает)|покажи|дай/,
  };
  ...
  function deterministicHead(text) {
    ...
    if (RE.ask.test(t) || /комисси|конверси|воронк/.test(t)) {
      ...
      const QUANTITY = /сколько|скольк|количеств|сумм|итог|всего|объ[её]м|покажи|дай|список|выведи|статус|состояни|что\s|чего\s/i;
