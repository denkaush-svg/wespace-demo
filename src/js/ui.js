...
(function (WS) {
  ...
  /* Чем объект хорош ДЛЯ КЛИЕНТА.

     Здесь стояла сортировка по `oppComm` — по НАШЕМУ вознаграждению. То есть то, что
     уходит клиенту как «подборка», было упорядочено по выгоде агентства и нигде об этом
     не говорилось. Брокер, отправляя такое, отправлял комиссионный список под видом
     рекомендации — и не знал об этом сам.

     Счёт собирается из того, на что смотрит покупатель, и КАЖДАЯ составляющая возвращается
     наружу: если порядок нельзя объяснить словами, ему нельзя доверять. Комиссия остаётся
     там, где ей место, — на ВНУТРЕННИХ экранах агента, а не в порядке письма клиенту. */
  function clientValue(o, c) {
    const dv = insDeviation(o);
    const m = insIndexFor(o) || {};
    // Цена метра против среза по району: ниже — лучше. Главный сигнал выгоды.
    const priceEdge = dv ? -dv.pct : 0;
    // Доходность района — то, ради чего инвестор вообще смотрит.
    const yieldPct = Number(m.доходностьПроцент || 0);
    // Запас к бюджету: выше бюджета — штраф, сильно ниже — тоже не подарок: значит не тот класс.
    const b = Number((c && c.budget) || 0);
    const ratio = b ? (o.price || 0) / b : 1;
    const fit = ratio > 1 ? -40 * (ratio - 1) : (ratio < 0.7 ? -10 * (0.7 - ratio) : 6 * (1 - Math.abs(0.9 - ratio)));
  ...
  function clientValueWhy(o, c) {
    ...
    const parts = [];
    if (v.dv && v.dv.pct <= -3) parts.push('на ' + Math.abs(v.dv.pct) + '% ниже среза по ' + v.area);
    else if (v.dv && v.dv.pct >= 5) parts.push('на ' + v.dv.pct + '% выше среза по ' + v.area);
    if (v.yieldPct) parts.push('доходность района ' + String(v.yieldPct).replace('.', ',') + '%');
    const b = Number((c && c.budget) || 0);
  ...
  function replyPicks(c) {
    ...
    return oppFreeObjects([]).filter((o) => (c.areas || []).indexOf(o.area) >= 0 &&
      ...
      .sort((a, b) => clientValue(b, c).score - clientValue(a, c).score).slice(0, 3);
  }
  ...
  function selectionCard(o, c) {
    ...
    return '<article class="sel-card">' +
      ...
      /* Почему этот объект вообще в подборке — из тех же чисел, по которым он туда попал.
         Порядок без объяснения — просьба верить на слово. */
      (c ? selectionFact('Почему в подборке', escAttr(clientValueWhy(o, c) || 'подходит по району и бюджету')) : '') +
      (o.match ? '<p class="sel-why">' + escAttr(o.match) + '</p>' : '') +
      /* usp — самая ценная строка в карточке и единственное, что отличает этот юнит от
         соседнего. Но это авторское описание, а не измеренный факт, — поэтому идёт с пометкой
         происхождения, а не вровень с ценой и площадью. */
      (o.usp ? '<p class="sel-usp"><span class="sel-usp-k">Описание объекта</span>' + escAttr(o.usp) + '</p>' : '') +
      '</div></article>';
  }
  ...
  /* Один документ — два входа. Раньше подборка собиралась ТОЛЬКО от входящего
     обращения. А брокер работает из карточки ЗАЯВКИ: там бюджет, районы и то, что уже
     предлагалось. Заявка без входящего (заведенная руками или пришедшая от партнёра)
     вообще не имела способа собрать клиенту оффер. */
  /* Планировка юнита. Дубайский брокер назвал это первым, что съедает время: клиент
  ...
  function openFloorplan(objId) {
    const o = (D().objects || []).find((x) => x.id === objId);
    if (!o) { WS.storeApi.toast('Объект не найден: ' + objId); return; }
    if (!WS.floorplan) { WS.storeApi.toast('Модуль схем не загружен'); return; }
    const n = WS.floorplan.note(o);
    openModal('Планировка · ' + escAttr(o.name),
      '<div class="fp-wrap">' + WS.floorplan.svg(o) + '</div>' +
      '<div class="fp-note">' +
        '<div class="fp-row"><span class="fp-k">Собрана из</span><span>' +
          escAttr(n.has.join(' · ') || 'данных объекта') + '</span></div>' +
        '<div class="fp-row fp-row--warn"><span class="fp-k">Чего здесь нет</span><span>' +
          escAttr(n.missing.join(' · ')) + '</span></div>' +
      '</div>' +
      '<div class="rw-prov" style="margin-top:10px">' + I('shield') +
        'Это схема соотношений, а не обмерный чертёж. Подпись стоит на самой картинке и уедет вместе с ней.</div>',
      '<button class="btn primary" data-act="copyFloorplan" data-obj="' + escAttr(o.id) + '">' + I('doc') + 'Скопировать картинку</button>' +
      '<button class="btn" data-obj="' + escAttr(o.id) + '">' + I('arrowRight') + 'Открыть объект</button>' +
      '<button class="btn" data-act="closeModal">Закрыть</button>', { wide: true });
  }
  function copyFloorplan(objId) {
    const o = (D().objects || []).find((x) => x.id === objId); if (!o) return;
    try {
      const url = WS.floorplan.dataUrl(o);
      if (navigator.clipboard && navigator.clipboard.writeText) navigator.clipboard.writeText(url);
      WS.storeApi.toast('Схема скопирована — вставьте в переписку', 'ok');
    } catch (e) { WS.storeApi.toast('Скопировать не вышло'); }
  }
  function openRequestOffer(reqId) {
    const r = requestById(reqId); if (!r) return;
    const c = oppClient(r.clientId); if (!c) { WS.storeApi.toast('У заявки нет карточки контакта'); return; }
    /* Уже предложенное по заявке важнее свежего подбора: брокер эти объекты уже
       отобрал руками, и пересобирать их алгоритмом значит тихо выбросить его работу. */
    const offered = (r.offered || []).map((x) => (D().objects || []).find((y) => y.id === x.id)).filter(Boolean);
    const objs = offered.length ? offered : selectionObjects(c);
    WS._sel = { req: r.id, ids: objs.map((o) => o.id) };
    selectionModal(c, objs, { reqId: r.id });
  }
  ...
  /* Тот же документ, собранный от заявки. Второй копии разметки здесь быть не должно:
     два документа разойдутся на первой же правке одного из них. */
  function selectionModal(c, objs, opts) {
    opts = opts || {};
    const asOf = (oppMarket()[0] || {});
    const head = '<div class="sel-head"><div class="sel-for">Подборка для ' + escAttr(c.name) + '</div>' +
      '<div class="sel-req">' + (c.goal ? escAttr(c.goal) + ' · ' : '') +
    ...
    const body = objs.length
      ? '<div class="sel-grid">' + objs.map((o) => selectionCard(o, c)).join('') + '</div>' +
        (selectionMeaning(objs, c).length
          ...
          : '') +
        '<div class="sel-prov">' + I('radar') + 'Срез рынка ' + escAttr(asOf.asOf || '—') +
        ', ' + escAttr(asOf.basis || 'иллюстративно') + '. Доступность подтверждена на дату проверки каждого объекта.</div>'
      : '<div class="sel-empty">' + I('warn') +
        '<div><b>Свободных объектов под этот запрос сейчас нет</b><span>Бюджет ' +
        (c.budget ? WS.AED(c.budget) : 'не назван') + ', районы ' + ((c.areas || []).join(', ') || 'не названы') +
        '. Собирать через партнёров или расширять район — придумывать варианты документ не будет.</span></div></div>';
    const foot = (objs.length
        ? '<button class="btn primary" data-act="sendReqOffer" data-req="' + escAttr(opts.reqId || '') + '">' +
          I('send') + 'Отправить клиенту</button>'
        : '<button class="btn" data-nav="partners">' + I('users') + 'Подобрать через партнёров</button>') +
      '<button class="btn" data-act="closeModal">Закрыть</button>';
    openModal('Подборка · то, что уйдёт клиенту', head + body, foot, { wide: true });
  }
  /* Отправка подборки, собранной от заявки. Проверка согласия — та же, что у любой
     другой адресной отправки в стенде, а не своя отдельная. */
  function sendRequestOffer(reqId) {
    const r = requestById(reqId); if (!r) return;
    const c = oppClient(r.clientId); if (!c) return;
    const audit = WS.audience.calculateAudience([{ id: r.id, clientId: c.id, channel: r.channel || c.channel }]);
    if (audit.excluded.length) {
      WS.storeApi.toast(((audit.excluded[0] || {}).reason || 'нельзя писать') + ' — отправка невозможна', 'warn');
      return;
    }
    const ids = (WS._sel && WS._sel.req === reqId) ? WS._sel.ids : [];
    const names = ids.map((id) => ((D().objects || []).find((o) => o.id === id) || {}).name).filter(Boolean);
    addEventEntry('request', r.id, { type: 'msg',
      text: 'Подборка отправлена — ' + (names.join(', ') || 'без объектов') + '. Отправка имитируется (DEMO).' });
    WS.storeApi.touch();
    closeModal();
  ...
  function conciergeStarters() {
    ...
    out.push(['users', silent ? 'Кто давно молчит' : 'С кем пора связаться', 'кто из клиентов давно не выходил на связь']);
    return out.slice(0, 4).map((s) =>
      '<button class="cg-start" data-cgask="' + escAttr(s[2]) + '">' + I(s[0]) + '<span>' + s[1] + '</span></button>').join('');
  }
  ...
  function conciergeThreadMain(st) {
    ...
    const inTri = /^inbox:/.test(String(WS.engine.activeThreadId() || ''))
      ? (D().inbox || []).find((x) => 'inbox:' + x.id === WS.engine.activeThreadId())
      : null;
    const triage = inTri ? inboxTriageCard(inTri) : '';
  ...
  /* Заявка, по которой работать больше нечего: клиент отказался или заявку закрыли. Её
     расхождение — история этой заявки, а не действующий факт другой сделки. Смысл узкий и живёт
     только здесь: reqStage() считает «закрытой» и заявку, целиком ушедшую в сделки, а такая как
     раз обязана отдавать своё расхождение дальше. */
  function requestAbandoned(r) {
    if (!r) return false;
    if (r.stage === 'lost' || r.stage === 'won') return true;
    if (r.closedAt) return true;
    return /закрыт|отказ|проигр|потерян/i.test(r.leadStatus || '');
  }
  /* Расхождение записывается на той записи, которая полем владеет: бюджет называет клиент, и
     ...
     заявки оно было видно, на сделке исчезало молча. Порядок поиска задан явно и не зависит от
     порядка записей в массиве: сначала своё, затем СОБСТВЕННОЕ основание сделки (requestId), и
     только потом — заявка того же клиента. Один `find` с «или» внутри уравнивал последние два, и
     побеждала первая заявка в массиве, а не основание сделки.
     Чужая заявка отдаёт расхождение, только пока сама жива: с закрытой оно перенеслось бы на
     новую сделку как действующее — утверждение об актуальности, которого никто не проверял. */
  function conflictSource(e) {
    ...
    const basis = e.requestId ? reqs.find((x) => x.id === e.requestId && cs[x.id]) : null;
    if (basis) return { cf: cs[basis.id], id: basis.id, own: false };
    const r = reqs.find((x) => cs[x.id] && x.clientId === e.clientId && !requestAbandoned(x));
  ...
  function conflictBlock(d) {
    ...
    /* Подпись говорит ровно то, что проверено. «Расхождение до сих пор не снято» — утверждение о
       сегодняшнем дне, и на расхождении с уже выбранным значением оно попросту неверно: выбор
       сделан, просто сделан не здесь. Поэтому два разных хвоста, а не один на оба случая. */
    const chosen = cf.chosen === 'a' || cf.chosen === 'b';
    const from = src.own ? ''
      : ' Записано на заявке клиента' + (cf.at ? ', ' + cf.at : '')
        + (chosen ? ' — там значение уже выбрано; для этой сделки выбор не подтверждали.'
                  : ' — выбор по нему пока не сделан.');
  ...
  function requestActions(r) {
    ...
    return [
      ...
      /* Сборка оффера — первое, что делают с заявкой, и единственное, чего здесь не было:
  ...
  /* Считается тем же кодом, что и в разборах Пульса, — и главное в нём то, что
     он спрашивает индекс СВОЕГО сегмента: офис против офисов, квартиру
     против квартир. Здесь до этого жила вторая, упрощённая копия без этой проверки,
     и офис в Business Bay показывал «−23% к району» против квартирного индекса.
     Нет индекса по своему сегменту — отклонения нет, и это честнее числа ни о чём. */
  function objPriceGap(o) {
    const dv = insDeviation(o);
    return dv ? dv.pct : null;
  }
  ...
  /* Разбор входящего открывает САМО обращение, а не подходящий по типу демо-сюжет.
     Прежний код выбирал сценарий по полю `ex`: у Sarah, написавшей «1BR investment unit in JVC,
     budget ~1.3M», запускался S15 с чужим русским запросом про 1,5 млн, а у обращения с
     возможным дублем сценария не находилось вовсе — и «Разобрать» открывало пустой Консьерж,
     потеряв текст, канал, время и сам повод. Оба случая проверены кликами на опубликованном
     стенде.

     Теперь кнопка несёт id записи, разговор привязывается к ней, а исходное сообщение
     кладётся в тред один раз — при повторном открытии оно не задваивается. Демо-сценарии
     остаются доступны из «Навигатора демо», где они и подписаны как сценарии. */
  const INBOX_EX_HINT = {
    qualify: 'Обращение не разобрано: нужно понять запрос и завести заявку.',
    duplicate: 'Похоже на дубль существующего контакта — сравните и решите.',
    unknown_object: 'Спрашивают об объекте, которого нет в инвентаре.',
    delivery_fail: 'Письмо не доставлено — адрес отклонил отправку.',
    noconsent: 'Нет согласия на переписку — связываться нельзя.',
  };
  function inboxTriageBtn(it) {
    return '<button class="btn xs" data-act="inboxTriage" data-inbox="' + escAttr(it.id) + '">' +
      I('sparkle') + 'Разобрать</button>';
  }
  function inboxDupCandidate(it) {
    // Кандидат ищется по имени из текста обращения, но СВЯЗЫВАЕТСЯ по id — показанное имя
    // не адресует запись, а решение о слиянии должно быть привязано к конкретному контакту.
    /* Искать по имени В КАВЫЧКАХ нельзя: там латиница («M. Ibragimov»), а контакт заведён
       кириллицей («Марат Ибрагимов»), и сравнение не совпадёт никогда. Ищем наоборот — какой
       из существующих контактов упомянут в тексте обращения; связь всё равно ставится по id. */
    const txt = String(it.text || '').toLowerCase();
    return (D().clients || []).find((c) => {
      const parts = String(c.name || '').toLowerCase().split(/\s+/).filter((w) => w.length > 3);
      return parts.length && parts.every((w) => txt.indexOf(w) >= 0);
    }) || null;
  }
  function inboxTriageCard(it) {
    const c = (D().clients || []).find((x) => x.id === it.clientId);
    const who = c ? c.name : 'Новый контакт';
    const assignee = it.assignee ? ((D().roster || []).find((x) => x.id === it.assignee) || {}).name : null;
    const ex = INBOX_EX_LABEL[it.ex];
    const dup = it.ex === 'duplicate' ? inboxDupCandidate(it) : null;
    const rows = [
      ['users', 'Кто', who],
      ['chat', 'Канал', it.channel || '—'],
      ['clock', 'Когда', it.at || '—'],
      ['check', 'Состояние', assignee ? 'назначено · ' + assignee : 'не назначено'],
    ].map((r) => '<div class="field"><div class="k">' + I(r[0]) + ' ' + r[1] + '</div><div class="v">' + escAttr(r[2]) + '</div></div>').join('');
    const dupBlock = dup
      ? '<div class="match" style="margin-top:10px">' + I('warn') +
        '<span>Похожий контакт в базе: <b>' + escAttr(dup.name) + '</b>' +
        (dup.phone ? ' · ' + escAttr(dup.phone) : '') + '. Решите: это он же или новый человек.</span></div>' +
        '<div class="qa-row" style="margin-top:8px">' +
        '<button class="btn xs" data-act="inboxDupMerge" data-inbox="' + escAttr(it.id) + '" data-cid="' + escAttr(dup.id) + '">' + I('link') + 'Это ' + escAttr(dup.name.split(' ')[0]) + '</button>' +
        '<button class="btn xs" data-act="inboxDupNew" data-inbox="' + escAttr(it.id) + '">' + I('plus') + 'Новый человек</button>' +
        '</div>'
      : '';
    return '<div class="card pad intriage">' +
      '<div class="section-label" style="margin:0 0 8px">Разбор обращения</div>' +
      (ex ? '<span class="badge ' + ex[1] + '">' + I('warn') + escAttr(ex[0]) + '</span>' : '') +
      '<div class="prov" style="margin:8px 0 10px"><span style="font-size:12.5px;color:var(--mut)">' +
        escAttr(INBOX_EX_HINT[it.ex] || 'Разберите обращение и решите, что с ним делать.') + '</span></div>' +
      rows + dupBlock +
  ...
  /* Решение по дублю пишется в саму запись и в историю контакта. Показанное имя ничего не
     ...
     ломает, а повторный разбор видит, что вопрос уже закрыт. */
  function inboxDupDecide(inboxId, clientId) {
    const it = (D().inbox || []).find((x) => x.id === inboxId);
    if (!it) { WS.storeApi.toast('Обращение не найдено: ' + inboxId); return; }
    if (clientId) {
      const c = (D().clients || []).find((x) => x.id === clientId);
      if (!c) { WS.storeApi.toast('Контакт не найден: ' + clientId); return; }
      it.clientId = clientId; it.dupOf = clientId; it.ex = null; it.resolution = 'merged';
      D().contactTimeline = D().contactTimeline || {};
      (D().contactTimeline[clientId] = D().contactTimeline[clientId] || []).push({
        ch: 'system', kind: 'ai', by: 'Разбор', at: 'только что', ord: 999,
        text: 'Обращение от ' + (it.at || '') + ' опознано как тот же человек — привязано к этому контакту.' });
      WS.storeApi.toast('Привязано к контакту: ' + c.name, 'ok');
    } else {
      it.ex = null; it.resolution = 'new_person';
      WS.storeApi.toast('Отмечено как новый человек', 'ok');
    }
    it.triagedAt = 'только что';
    WS.storeApi.save(); WS.storeApi.emit();
  }
  function openInboxTriage(inboxId) {
    const it = (D().inbox || []).find((x) => x.id === inboxId);
    if (!it) { WS.storeApi.toast('Обращение не найдено: ' + inboxId); return; }
    const c = (D().clients || []).find((x) => x.id === it.clientId);
    const tid = 'inbox:' + it.id;
    WS.engine.bindThread(tid, (c ? c.name + ' · ' : '') + 'обращение ' + (it.at || ''), 'mail');
    const th = WS.engine.activeThread();
    // Исходное сообщение кладётся ОДИН раз: повторное открытие не должно задваивать переписку.
    if (th && !(th.items || []).length) {
      WS.engine.pushMsg('<div class="msg user fadeup"><div class="who">' +
        escAttr(c ? c.name : 'Новый контакт') + ' · ' + escAttr(it.channel || '') + ' · ' + escAttr(it.at || '') +
        '</div><div class="bubble">' + escAttr(it.text || '') + '</div></div>', tid);
    }
    S().cgDock = false;
