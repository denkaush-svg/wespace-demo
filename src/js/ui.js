...
(function (WS) {
  ...
  const PROSPECT_RULES = [
    ...
    /* 5. Возможность, запертая комплаенсом. Бюджет есть, интерес есть, а писать нельзя:
          согласия на связь нет. Это единственный вид возможности, где первое действие —
          не предложение, а получение права предлагать. */
    { key: 'consent_block', label: 'Возможность заперта согласием', icon: 'lock',
      find(claimed, used) {
        return (D().clients || []).filter((c) => c.consent === false && c.budget)
          .map((c) => {
            const area = (c.areas || [])[0] || '';
            const m = oppMarketOf(area);
            /* Допуск 10%: объект на четыре процента дороже бюджета — это предмет разговора,
               а не «в бюджете ничего нет». Допуск назван в самой строке, а не спрятан. */
            const fit = (D().objects || []).filter((o) => o.area === area &&
              o.availability === 'available' && (o.price || 0) <= c.budget * 1.1);
            return {
              clientId: c.id, client: c.name, role: oppRoleWord(c),
              tone: 'stop',
              why: 'Бюджет назван, район назван, а адресно писать нельзя: согласия на связь нет. ' +
                'Пока оно не получено, любая отправка — нарушение, и лид стоит на месте не потому, ' +
                'что клиент остыл.',
              basis: [['Согласие на связь', 'не получено — адресные отправки заблокированы'],
                ['Бюджет и район', WS.AED(c.budget) + ' · ' + (area || 'район не назван') +
                  (c.horizon ? ' · горизонт ' + c.horizon : '')],
                ['Подходит в инвентаре', fit.length
                  ? fit.length + ' ' + plural(fit.length, 'объект', 'объекта', 'объектов') + ' в бюджете с допуском 10%'
                  : 'в бюджете с допуском 10% сейчас ничего нет'],
                m ? ['Рынок · ' + m.район, WS.AED(m.ценаЗаМетр) + ' за м², доходность ' +
                  m.доходностьПроцент + '%, +' + m.изменениеЗаГодПроцент + '% за год'] : null,
              ].filter(Boolean),
              offer: 'Сначала — согласие на связь; подборка идёт следом и уже законно',
    ...
    /* 6. Бюджет совпал с целым объектом. Крупный собственник и свободный блок: цена метра ниже
    ...
    /* 7. Объект проигранной сделки снова свободен. Причина ухода могла быть в чём угодно, но
    ...
    /* 8. Отчёт собственнику как готовый повод. Разговор всё равно состоится в назначенный срок
  ...
  // Стадии, которые на доску не попадают намеренно: `new`/`qual` разбираются во «Входящих»,
  // `closed` уже представлена своими сделками — посчитать её ещё раз значило бы посчитать дважды.
  const BAND_OFF_BOARD = ['new', 'qual', 'closed'];
