...
(function (WS) {
  ...
  const store = {
    ...
    contactsFilters: CONTACT_FILTERS_DEFAULT(),
    ...
    clientsTab: 'deals', rightPane: null, pulseTab: 'deals', pulseDay: 'today', dayView: 'table', prospIdx: 0, prospDeck: false, pulseOpen: {}, contactsChat: false, navRail: false, dealsView: null, navOpen: false, cgRailOpen: false, cgCtx: [], cgMenu: null, cgMode: 'auto', cgDepth: 'think',
  ...
  function resetAll() {
    ...
    store.contactsFilters = CONTACT_FILTERS_DEFAULT();
    store.cgGroupCollapse = {};
    store.companiesFilters = { client: 'all' };
    store.eventsPlayed = []; store.feedback = []; store.signals = []; store.dayStep = 0;
    store.clientsTab = 'deals'; store.pulseTab = 'deals'; store.contactsChat = false; store.navRail = false; store.pulseDay = 'today'; store.dayView = 'table'; store.prospIdx = 0; store.prospDeck = false; store.pulseOpen = {}; store.dealsView = null; store.cgCtx = []; store.cgMenu = null; store.cgMode = 'auto'; store.cgDepth = 'think';
